...
write.csv(metab_final$e_data, "Data/LC/normalized_pos_preinc.csv", quote = F, row.names = F)
...
plot(statResAnova)
write.csv(statResAnova, "Data/LC/stats_pos_preinc.csv", quote = F, row.names = F)
...
write.csv(metab_final$e_data, "Data/LC/normalized_pos_inc.csv", quote = F, row.names = F)
...
plot(statResAnova)
write.csv(statResAnova, "Data/LC/stats_pos_inc.csv", quote = F, row.names = F)
...
edata2<-edata%>%
  ...
  select(Name,X36_Pre_2_1, X22_Pre_2_2, X20_Pre_2_3, X35_Pre_6_1,	X7_Pre_6_2, X2_Pre_6_3)
emeta<-edata%>%
  select(m.z,RT..min.,Name,Tags,RefMet.Nomenclature,Main.class,Sub.class,Formula,Annot..DeltaMass..ppm.,Calc..MW,Reference.Ion
  )
...
plot(statResAnova)
write.csv(statResAnova, "Data/LC/stats_pos_preincONLY.csv", quote = F, row.names = F)
